...
async function openApp(){if(!token)return showAuth('يجب تسجيل الدخول أولًا.');try{const r=await fetch(API+'/api/v1/app',{headers:{Authorization:`Bearer ${token}`},cache:'no-store'});let d={};try{d=await r.json()}catch{}if(!r.ok){if(d.code==='TRIAL_EXPIRED')return showExpired(d.error);throw new Error(d.error||'تعذر فتح المنصة.')}document.open();document.write(d);document.close()}catch(e){if(e.code==='TRIAL_EXPIRED')showExpired(e.message);else message(e.message)}}
function startTimer(ms){clearInterval(window.__trialTimer);let end=Date.now()+Math.max(0,ms);const tick=()=>{let left=Math.max(0,end-Date.now());const sec=Math.ceil(left/1000);const m=Math.floor(sec/60),s=sec%60;if($('timer'))$('timer').textContent=`التجربة المتبقية: ${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`;if(left<=0){clearInterval(window.__trialTimer);showExpired('انتهت الفترة التجريبية (30 دقيقة). أرسل وصل الدفع عبر واتساب لتفعيل الحساب.')}};tick();window.__trialTimer=setInterval(tick,1000)}
